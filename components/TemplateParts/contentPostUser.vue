          ...
          <div class="post-cover">
            <div class="PostFeaturedMedia relative w-full h-full">
              <div class="block absolute inset-0">
                <img :alt="postItem.post.title" class="object-cover w-full h-[200px]" :src="`${$config.public.appBaseUrl}/storage/posts/${postItem.post.image}`" />
              </div>
              <PostTypeIcon />
            </div>
          </div>
              ...
              >
                <div
                    ...
                    <img alt="Falconar Agnes" class="absolute inset-0 w-full h-full object-cover"
                        :src="postItem.post.user.profile == null ? '/2.webp' :`${appBaseUrl}/storage/avatars/${postItem.post.user.profile}`" />
                </div>
  
                <span v-if="postItem.post.user != null" class="block text-neutral-700 hover:text-black dark:!text-neutral-300 dark:hover:text-white font-medium">
                  {{postItem.post.user.name}} {{postItem.post.user.family}}
                </span>
              </div>
            <span
