    ...
    <article>
      ...
      >
        <div class="block flex-shrink-0 relative w-full rounded-t-3xl overflow-hidden z-10 aspect-w-5 aspect-h-3">
          <div class="post-cover">
            <div class="PostFeaturedMedia relative w-full h-full">
              <nuxt-link :to="`/post/${postItem.slug}-${postItem.id}`" class="block absolute inset-0">
                <img alt="featured" class="object-cover h-full w-full" :src="`${$config.public.appBaseUrl}/storage/posts/${postItem.image}`" />
              </nuxt-link>
              <PostTypeIcon />
            </div>
          </div>
        ...
        <div class="p-4 flex flex-col space-y-3">
          ...
          >
              <nuxt-link :to="`/post/${postItem.slug}-${postItem.id}`"
